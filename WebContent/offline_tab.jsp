...
<head>
	...
	<title>offline Assessment</title>
	<style>
	body{
		font-family:serif;
	}
...
<body>
	...
	<div id="main">
		...
		<nav class="nbar">
			...
			<a href="offline_tab.jsp" class="nbtn" id="thispage" target="_self"><b>Offline assessment</b></a>
		</nav><hr>
		
				<!-- file-list -->
		<%
			PreparedStatement pst = conn.prepareStatement("select* from assessment where class_code = ?");
		...
		%>
			<div class="expand">
				<button type="button" class="card">
					<img src="./images/Assessment.png" alt="DOC" style="width:40px; height:45px;">
					<span><%=result.getString("title") %></span>
					<span class="name">
						max marks : <%=result.getInt("max_marks") %>
					</span>
			 	</button>
			 
			 	<div class="submit">
					<hr style="margin-bottom:20px"><%=result.getString("description") %><hr style="margin-top:20px">
			 </div>
		 </div>
		<%
				}
		...
		<span style="font-size:36px;padding:10px">Offline Assessment</span><hr>
		<a href="javascript:void(0)" class="close" onclick="document.getElementById('div1').style.display='none'">&times;</a>	
			
			<form action="<%= request.getContextPath()%>/AssessServlet" method ="post" id="assign">
