...
<body>
	...
	<div id="main">
		...
		<nav class="nbar">
			...
			<a href="classwork_tab.jsp" class="nbtn" id="thispage" target="_self"><b>Classwork</b></a>
		...
		<!-- file-list -->
		   ...
		   <%
				PreparedStatement pst = conn.prepareStatement("select* from classwork where class_code = ?");
				pst.setInt(1, Fetch.getClass_code());
				ResultSet result = pst.executeQuery();
				while(result.next()) {
			 %>
			<a href = "DownloadServlet?file_name=<%=result.getString("file_name") %>" class="tab">
			<div class=card>
				<img src="./images/Books.png" alt="DOC" style="width:40px; height:40px;">
				<%=result.getString("file_name") %>
				<span class="name"><%=result.getString("fname") %>
				<%=result.getString("lname") %><br><%=result.getDate("added_date") %></span>
			 </div>
           </a>
			<%
				}
