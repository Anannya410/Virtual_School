...
<body>
	...
	<div id="mySidenav" class="sidenav">
		...
		<%
			...
			while(result.next()) {
		%>
			<a href = "FetchAssignServlet?title=<%=result.getString("title") %>" class="tab">
			<div class= "card">
			...
			<span style="font-size:15px"><%=result.getString("description") %></span>
			</div>
			</a>
		...
		<!-- file-list -->
		<%
			PreparedStatement st = conn.prepareStatement("select* from assessment where class_code = ?");
			st.setInt(1, Fetch.getClass_code());
			ResultSet rst = st.executeQuery();
			while(rst.next()) {
		%>
			<a href = "FetchAssessServlet?title=<%=rst.getString("title") %>" class="tab">
			<div class= "card">
			<img src="./images/Assessment.png" alt="DOC" style="width:40px; height:45px;">
			<%=rst.getString(2) %>
				<span class="name">
					max marks : <%=rst.getInt("max_marks") %>
				</span><br>
			<span style="font-size:15px"><%=rst.getString("description") %></span>
			</div>
			</a>
